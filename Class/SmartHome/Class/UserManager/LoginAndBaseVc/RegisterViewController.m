...
-(void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    if (textField == txtPhoneNumber) {
        viewBackground.frame = CGRectMake(0, -20, SCREEN_CGSIZE_WIDTH, SCREEN_CGSIZE_HEIGHT);
        
    }
    if (textField == txtPassWord) {
        viewBackground.frame = CGRectMake(0, -100, SCREEN_CGSIZE_WIDTH, SCREEN_CGSIZE_HEIGHT);
    }
    else if(textField == txtPhoneCode)
    {
        viewBackground.frame = CGRectMake(0, -60, SCREEN_CGSIZE_WIDTH, SCREEN_CGSIZE_HEIGHT);
    }
}
...
- (void)viewDidLoad {
    ...
    if (![currentLanguage isEqualToString:@"zh-Hans"]) {
        txtPhoneCode.secureTextEntry = YES;
    }
    ...
    secondTime = 0;
    //添加倒计时的Label
    lblTime = [[UILabel alloc] initWithFrame:CGRectMake(215, 208, 88, 40)];
    imgTime = [[UIImageView alloc] initWithFrame:CGRectMake(215, 208, 88, 40)];
...
- (IBAction)cmd_btn_readSelect:(id)sender
{
    ...
    if(isClick == NO)
    {
        [btnSelect setBackgroundImage:[UIImage imageNamed:@"Register_btn_select_h.png"] forState:UIControlStateNormal];
        isClick=YES;
    }
    else
    {
        [btnSelect setBackgroundImage:[UIImage imageNamed:@"Register_btn_select.png"] forState:UIControlStateNormal];
        isClick=NO;
    }
...
{
    YXMTermsOfServiceViewController *tos = [[YXMTermsOfServiceViewController alloc] init];
...
-(void)regInfoSubmit:(UIButton *)sender{
    //测试afnetwroking
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
    manager.requestSerializer = [AFHTTPRequestSerializer serializer];
    manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    NSDictionary *parameters = @{@"username": txtPhoneNumber.text,@"password": txtPassWord.text,@"email": [NSString stringWithFormat:@"%@@antbang.com",txtPhoneNumber.text],@"mobile": txtPhoneNumber.text};
    [manager POST:[NSString stringWithFormat:@"%@/usermanager.php?act=reg",URL_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        UIAlertView* alert=[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"regreturninfotitle", nil)
                                                      message:echo
                                                     delegate:self
                                            cancelButtonTitle:NSLocalizedString(@"sure", nil)
                                            otherButtonTitles:nil, nil];
